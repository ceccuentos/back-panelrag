...
class CallbackEvent(BaseModel):
    ...
    def get_retrieval_message(self) -> dict | None:
        if self.payload:
            ...
            if nodes:
                msg = f"Recupera {len(nodes)} fuentes para usar en el contexto de la consulta"
            else:
                msg = f"Recuperando contexto para la consulta: '{self.payload.get('query_str')}'"
            return {
                "type": "events",
                "data": {"title": msg},
            }
    ...
    def get_tool_message(self) -> dict | None:
        ...
        if func_call_args is not None and "tool" in self.payload:
            ...
            return {
                "type": "events",
                "data": {
                    "title": f"Invocando tool: {tool.name} with inputs: {func_call_args}",
                },
            }
